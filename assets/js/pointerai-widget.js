...
(() => {
  ...
  function sanitizeConfig(config) {
    ...
    return {
      ...
      launcherIconStyle: config.launcherIconStyle === "bubble" ? "bubble" : DEFAULTS.launcherIconStyle,
  ...
  var PointerAIWidgetInstance = class {
    ...
    hasRuntimeAuth() {
      var _a;
      const sessionState = this.client.getSessionTokenState();
      return Boolean(((_a = this.client.endUserToken) == null ? void 0 : _a.trim()) || sessionState.token);
    }
    bindEvents() {
    ...
    async bootstrap() {
      ...
      try {
        ...
        if (endUserToken) {
          ...
          this.setAnonymousIdentity(createAnonUid("pa"));
        }
        await this.ensureSession();
    ...
    async ensureSession() {
      if (this.sessionUid) return;
      if (!this.hasRuntimeAuth() && !this.anonUid) {
        this.setAnonymousIdentity(createAnonUid("pa"));
      }
      const payload = {
    ...
    setAnonymousIdentity(anonUid) {
      const normalized = anonUid.trim();
      if (!normalized) return;
      this.rebindStorageScope(null, normalized);
      this.anonUid = normalized;
      this.persistAnonUid(normalized);
    }
    ...
    buildMarkup() {
      ...
      const launcherLabel = this.config.launcherLabel.trim();
      const launcherLabelHtml = launcherLabel ? `<span>${this.escapeHtml(launcherLabel)}</span>` : "";
      const launcherClass = launcherLabel ? "pa-launcher" : "pa-launcher pa-launcher-icon-only";
      const launcherAriaLabel = launcherLabel || this.config.title;
      const logoHtml = this.config.logoUrl ? `<img class="pa-logo" src="${this.escapeHtml(this.config.logoUrl)}" alt="logo" />` : "";
      let launcherIconHtml = `<span class="pa-launcher-icon">${this.escapeHtml(this.config.launcherIcon)}</span>`;
      if (this.config.launcherIconStyle === "bubble") {
        launcherIconHtml = '<span class="pa-launcher-bubble" aria-hidden="true"></span>';
      } else if (this.config.launcherIconUrl) {
        launcherIconHtml = `<img class="pa-launcher-icon-img" src="${this.escapeHtml(this.config.launcherIconUrl)}" alt="chat" />`;
      }
      return `
<style>
  :host { all: initial; }
...
<div class="pa-root">
  <button class="${launcherClass}" type="button" aria-label="${this.escapeHtml(launcherAriaLabel)}">${launcherIconHtml}${launcherLabelHtml}</button>
  <div class="pa-panel">
    <div class="pa-header">
